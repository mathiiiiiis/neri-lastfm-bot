// ==== topsongs ====
// lists users most played songs for timeframe
//
// > name: topsongs
...
async function topsongs(message) {
  ...
  let top;
  try {
    top = await lastfm.getTopSongs(username, period, count);
  } catch (err) {
    if (err.lastfmCode === 6) {
  ...
  const raw = top && top.track;
  const songs = Array.isArray(raw) ? raw : raw ? [raw] : [];

  if (!songs.length) {
    await message.reply(
      `[#37a0fc]**${username}** has no top songs for ${timeframe.label(period)}`
    );
    return;
  }

  const lines = songs.map((a, i) => {
  ...
  const head = `**Top songs** • ${timeframe.label(period)} • ${username}`;
